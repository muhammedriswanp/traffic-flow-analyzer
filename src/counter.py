...
class VehicleCounter:
    """Counts vehicles crossing a horizontal line using ByteTrack IDs."""

    def __init__(self, config: CounterConfig):
        self.cfg = config
        self._last_y: dict[int, float] = {}           # track_id → last centre_y
        self._crossed: set[int] = set()               # IDs that already crossed
        self._counts: dict[str, dict[str, int]] = defaultdict(lambda: {"in": 0, "out": 0})
        self.crossings: list[dict] = []               # event log for reporting

    # ── main update ──────────────────────────────────────────────────────────
    def update(self, results, frame_idx: int = 0) -> None:
        """
        Feed one model.track() result.

        Args:
            results:   single Results object from model.track(frame, persist=True)[0]
            frame_idx: current frame number (stored in crossing events)
        """
    ...
    def draw_overlay(self, frame: np.ndarray) -> None:
        ...
        cv2.line(
            frame,
            (0, self.cfg.line_y), (w, self.cfg.line_y),
            self.cfg.line_color, self.cfg.line_thickness,
        )
        cv2.putText(
            frame, "COUNT LINE",
            (8, self.cfg.line_y - 6),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, self.cfg.line_color, 1, cv2.LINE_AA,
        )
        ...
        board_w = 220
        board_h = len(lines) * line_h + pad * 2
